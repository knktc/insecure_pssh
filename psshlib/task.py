...
class Task(object):
    ...
    of the process.
    """
    def __init__(self, host, port, user, password, cmd, opts, stdin=None):
    ...
    #create a tempfile which contains a short shell script to print password
    #this script will be set to SSH_ASKPASS
    def make_passfile(self, preffix, host, password):
        characters = ("abcdefghijklmnopqrstuvwxyz" +
                  "ABCDEFGHIJKLMNOPQRSTUVWXYZ" +
                  "0123456789_")
        temp_filename = preffix + string.join(random.sample(characters, 6)).replace(' ', '')
        getpass_path = os.path.join(tempfile.gettempdir(), temp_filename)
        try:
            temp_shellfile = open(getpass_path, 'wb')
            temp_shellfile.write("#!/bin/sh\n echo '%s'" % password)
            temp_shellfile.close()
            os.chmod(getpass_path, 0777)
            return getpass_path
        except:
            return False
    ...
    def start(self, nodenum, iomap, writer):
        ...
        # a temp shell script to get password
        
        #create a script to print password
        #CRITICAL!!! INSECURITY!!!
        self.getpass_path = self.make_passfile('tmp', self.host, self.password)
        environ['SSH_ASKPASS'] = ("%s" % self.getpass_path)
    ...
    def del_getpass(self, getpass_path):
        if os.path.isfile(getpass_path):
            os.remove(getpass_path)
